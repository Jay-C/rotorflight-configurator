...
TABS.motors.initialize = function (callback) {
    ...
    function process_html() {
        ...
        const motorOverrideElements = $('.overridesEnabled');
        ...
        enableMotorOverrideSwitch.change(function () {
            ...
            for (let i = 0; i < FC.CONFIG.motorCount; ++i) {
                const motorSwitch = $(`.motorOverride${i} .motorOverrideThrottle input`);
                motorSwitch.val(0).change();
            }

            motorOverrideElements.toggle(checked);
        });

        enableMotorOverrideSwitch.prop('checked', !FC.CONFIG.motorOverrideDisabled);
        motorOverrideElements.toggle(!FC.CONFIG.motorOverrideDisabled);
